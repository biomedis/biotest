...
public abstract class GenericDAO<T extends IEntity> implements IDAO<T>
{
        ...
        class Column
        {
            ...
            private void setFieldEntityType(Class<?> fieldEntityType)
            {
                ...
                try {
                    ...
                    this.methodGet=persistentClass.getMethod("get"+st, new Class[]{});//фищем геттер по имени поля
                } catch (NoSuchMethodException e) {
                    e.printStackTrace();
                    throw new RuntimeException("В сущности отсутствует метод установки или получения в соответствии с аннотированным полем. "+st );
                }
    ...
    {
        if(this.isExistTable()) {Log.v("Таблица "+this.entityBuilder.getTableName()+" уже существует");return;}
        ...
        switch (fieldDbType) {
            ...
            case Cursor.FIELD_TYPE_INTEGER:
                ...
                if (entityBuilder.getColumns().get(nameField).getFieldEntityType().equals(Date.class)) {
                    ...
                    cv.put(nameField, (long) dt.getTime());
...
//TODO Разобраться - если подаются в сущности поля с null
   ...
   synchronized public Integer save(T entity,boolean safely) throws Exception {
     ...
     * @param inTransaction если true то будут транзакция на этот вызов, если false, то мы сами должны управлять транзакцией. Например чтобы объеденить несколько вызовов.
     * @return
     */
    @Override
